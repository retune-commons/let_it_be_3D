...
from .plotting import Alignment_Plot_Crossvalidation, Predictions_Plot, Calibration_Validation_Plot, Triangulation_Visualization
...
from .angles_and_distances import add_reprojection_errors_of_all_test_position_markers, set_distances_and_angles_for_evaluation, fill_in_distances, add_all_real_distances_errors, set_angles_error_between_screws_and_plane
...
class Calibration(Triangulation_Calibration):
    ...
    def run_calibration(
        ...
        if (not test_mode) or (test_mode and not self.calibration_output_filepath.exists()):
...
class Triangulation(Triangulation_Calibration):
    ...
    def run_triangulation(
        self, calibration_toml_filepath: Path, save_first_frame: bool = False, test_mode: bool=False
    ):
    ...
    def _fake_missing_files(self, cams_in_calibration: List[str], framenum: int, markers: List[str])->None:
        for cam in cams_in_calibration:
            try:
                ...
                h5_output_filepath = self.output_directory.joinpath(self.csv_output_filepath.stem + f"empty_{cam}.h5")
    ...
    def _validate_test_position_marker_ids(self, test_position_markers_df_filepath: Path, framenum: int, add_missing_marker_ids_with_0_likelihood: bool = True) -> None:
        ...
        test_position_markers_df.to_hdf(test_position_markers_df_filepath, 'empty', mode = 'w')
...
class Triangulation_Positions(Triangulation):
    ...
    def get_marker_predictions(self) -> None:
        ...
        for cam in self.metadata_from_videos.values():
            ...
            Predictions_Plot(image = cam.filepath, predictions = h5_output_filepath, output_directory=self.output_directory, cam_id=cam.cam_id)

    def evaluate_triangulation_of_test_position_markers(self, show_3D_plot: bool = True, verbose: int = 1) -> None:        
        self.anipose_io = add_reprojection_errors_of_all_test_position_markers(anipose_io=self.anipose_io)
        self.anipose_io = set_distances_and_angles_for_evaluation(self.test_positions_gt, self.anipose_io)
        gt_distances = fill_in_distances(self.test_positions_gt["distances"])
        self.anipose_io = add_all_real_distances_errors(anipose_io=self.anipose_io,
                                                        test_positions_distances=gt_distances)
        self.anipose_io = set_angles_error_between_screws_and_plane(self.test_positions_gt["angles"], self.anipose_io)
        ...
        if show_3D_plot:
            Calibration_Validation_Plot(p3d = self.anipose_io['p3ds'][0], bodyparts = self.anipose_io['bodyparts'], output_directory = self.output_directory, marker_ids_to_connect=self.test_positions_gt["marker_ids_to_connect_in_3D_plot"], plot=True, save=True)
            
    def _create_csv_filepath(self) -> None:
        filepath_out = self.output_directory.joinpath(
            f"{self.recording_date}.csv"
        )
        return filepath_out