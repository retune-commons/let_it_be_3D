...
def set_distances_and_angles_for_evaluation(parameters_dict, anipose_io):
    if "distances" in parameters_dict:
        anipose_io = _set_distances_from_configuration(parameters_dict["distances"], anipose_io)
    else:
        print(
...
def _set_distances_from_configuration(distances_to_compute, anipose_io):
    conversion_factors = _get_conversion_factors_from_different_references(anipose_io,
                                                                                distances_to_compute)
    _add_distances_in_cm_for_each_conversion_factor(anipose_io, conversion_factors)
    return anipose_io

def add_all_real_distances_errors(anipose_io: Dict, test_positions_distances: Dict) -> Dict:
    ...
    anipose_io = _add_distances_in_cm_for_each_conversion_factor(anipose_io=anipose_io,
                                                                      conversion_factors=all_distance_to_cm_conversion_factors)
    anipose_io = _add_distance_errors(anipose_io=anipose_io,
...
def _add_distances_in_cm_for_each_conversion_factor(anipose_io: Dict, conversion_factors: Dict) -> Dict:
    anipose_io['distances_in_cm'] = {}
    for reference_distance_id, conversion_factor in conversion_factors.items():
        anipose_io['distances_in_cm'][reference_distance_id] = _convert_all_xyz_distances(
...
def _compute_differences_between_triangulated_and_gt_distances(triangulated_distances: Dict,
...
def _compute_differences_between_triangulated_and_gt_angles(gt_angles: Dict, anipose_io) -> Dict[str, float]:
    """
...
def _computes_angles(angles_to_compute, anipose_io) -> Dict[str, float]:
    ...
    for angle, markers_dictionary in angles_to_compute.items():
        if len(markers_dictionary["marker"]) == 3:
            pt_a = _get_vector_from_label(label=markers_dictionary["marker"][0], anipose_io=anipose_io)
            pt_b = _get_vector_from_label(label=markers_dictionary["marker"][1], anipose_io=anipose_io)
            pt_c = _get_vector_from_label(label=markers_dictionary["marker"][2], anipose_io=anipose_io)
            triangulated_angles[angle] = _get_angle_between_three_points_at_PointA(PointA=pt_a,
                                                                                        PointB=pt_b,
                                                                                        PointC=pt_c)
        elif len(markers_dictionary["marker"]) == 5:
            pt_a = _get_vector_from_label(label=markers_dictionary["marker"][2], anipose_io=anipose_io)
            pt_b = _get_vector_from_label(label=markers_dictionary["marker"][3], anipose_io=anipose_io)
            pt_c = _get_vector_from_label(label=markers_dictionary["marker"][4], anipose_io=anipose_io)
            plane_coord = _get_coordinates_plane_equation_from_three_points(PointA=pt_a,
                                                                                 PointB=pt_b,
                                                                                 PointC=pt_c)
            N = _get_vector_product(A=plane_coord[0], B=plane_coord[2])

            pt_d = _get_vector_from_label(label=markers_dictionary["marker"][0], anipose_io=anipose_io)
            pt_e = _get_vector_from_label(label=markers_dictionary["marker"][1], anipose_io=anipose_io)
            triangulated_angles[angle] = _get_angle_between_two_points_and_plane(PointA=pt_d, PointB=pt_e, N=N)
        else:
...
def set_angles_error_between_screws_and_plane(gt_angles, anipose_io):
    ...
    :return:
    """
    anipose_io["angles_error_screws_plan"] = _compute_differences_between_triangulated_and_gt_angles(
        gt_angles, anipose_io)
    return anipose_io
...
def _convert_all_xyz_distances(anipose_io: Dict, conversion_factor: float) -> Dict:
    ...
    for marker_id_a, marker_id_b in marker_id_combinations:
        ...
        xyz_distance = _get_xyz_distance_in_triangulation_space(marker_ids=(marker_id_a, marker_id_b),
...
def _get_conversion_factors_from_different_references(anipose_io: Dict,
...
def _get_xyz_to_cm_conversion_factor(reference_marker_ids: Tuple[str, str], distance_in_cm: Union[int, float],
                                     df_xyz: pd.DataFrame) -> float:
    distance_in_triangulation_space = _get_xyz_distance_in_triangulation_space(marker_ids=reference_marker_ids,
                                                                                    df_xyz=df_xyz)
...
def _get_angle_from_law_of_cosines(length_a: float, length_b: float, length_c: float) -> float:
    cos_angle = (length_c ** 2 + length_b ** 2 - length_a ** 2) / (2 * length_b * length_c)
    return math.degrees(math.acos(cos_angle))

def _get_angle_between_three_points_at_PointA(PointA: np.array, PointB: np.array, PointC: np.array) -> float:
...
def _get_coordinates_plane_equation_from_three_points(PointA: np.array, PointB: np.array,
...
def _get_Richtungsvektor_from_two_points(PointA: np.array, PointB: np.array) -> np.array:
...
def _get_angle_between_two_points_and_plane(PointA: np.array, PointB: np.array, N: np.array) -> float:
...
def _get_vector_from_label(label: str, anipose_io: Dict) -> np.array:
    return np.asarray([anipose_io['df_xyz'][label + '_x'], anipose_io['df_xyz'][label + '_y'],
                       anipose_io['df_xyz'][label + '_z']])

def _get_distance_between_plane_and_point(N: np.array, PointOnPlane: np.array,
