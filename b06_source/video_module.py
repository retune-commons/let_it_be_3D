...
import numpy as np
from gait3d.camera_intrinsics import IntrinsicCalibratorFisheyeCamera, IntrinsicCalibratorRegularCameraCharuco
from gait3d.utils import load_single_frame_of_video
...
class VideoMetadata:
    ...
    def intrinsic_calibrations_directory(self) -> Path:
        return Path('G:Konstantin/Data/Checkerboard_Calibrations/')
    ...
    def _get_intrinsic_parameters(self, config_filepath: Path, load_calibration: bool, max_frame_count: int) -> None:
        if self.charuco_video:
            if self.fisheye:
                try:
                    intrinsic_calibration_filepath = self.intrinsic_calibrations_directory.joinpath('Bottom_checkerboard_intrinsic_calibration_results_1000frames.p')
...
class VideoInterface:
    ...
    def inspect_intrinsic_calibration(self, frame_idx: int=0) -> None:
        distorted_input_image = load_single_frame_of_video(filepath = self.metadata.filepath, frame_idx = frame_idx)
    ...
    def run_synchronizer(self, synchronizer, use_gpu: bool) -> None:
        # synchronizer type hint = Synchronizer
        synchronizer_object = synchronizer()
        self.synchronized_object_filepath = synchronizer_object.run(video_metadata = self.metadata, use_gpu = use_gpu)
    ...
    def _export_as_aniposelib_Camera_object(self) -> ap_lib.cameras.Camera:
        if self.metadata.fisheye:
            camera = ap_lib.cameras.FisheyeCamera(name = self.metadata.cam_id,
                                           matrix = self.metadata.intrinsic_calibration['K'],
                                           dist = self.metadata.intrinsic_calibration['D'],
                                           extra_dist = False)
        else:                                   
            camera = ap_lib.cameras.Camera(name = self.metadata.cam_id,
                                   matrix = self.metadata.intrinsic_calibration['K'],
                                   dist = self.metadata.intrinsic_calibration['D'],
                                   extra_dist = False)
        return camera
    ...
    def _undistort_fisheye_image_for_inspection(self, image: np.ndarray) -> np.ndarray:
        k_for_fisheye = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(self.metadata.intrinsic_calibration['K'], 
                                                                               self.metadata.intrinsic_calibration['D'], 
                                                                               self.metadata.intrinsic_calibration['size'], 
                                                                               np.eye(3), 
                                                                               balance=0)
        map1, map2 = cv2.fisheye.initUndistortRectifyMap(self.metadata.intrinsic_calibration['K'], 
                                                         self.metadata.intrinsic_calibration['D'], 
                                                         np.eye(3), 
                                                         k_for_fisheye, 
                                                         (self.metadata.intrinsic_calibration['size'][1], self.metadata.intrinsic_calibration['size'][0]), 
                                                         cv2.CV_16SC2)
