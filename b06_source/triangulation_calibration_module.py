...
class Calibration():
    def __init__(self, calibration_directory: Path, output_directory: Optional[Path] = None, load_calibration: bool = False, max_frame_count: int = 300, use_gpu: bool = True) -> None:
        self._create_video_objects(calibration_directory = calibration_directory, load_calibration = load_calibration, max_frame_count = max_frame_count, use_gpu = use_gpu)
    ...
    def _create_video_objects(self, calibration_directory: Path, max_frame_count: int, load_calibration: bool, use_gpu: bool = True) -> None:
        ...
        config_filepath = calibration_directory.joinpath('config.json')
        charuco_metadata = [VideoMetadata(filepath = filepath, config_filepath = config_filepath, load_calibration = load_calibration, max_frame_count = max_frame_count) for filepath in charuco_videofiles]
    ...
    def _validate_unique_cam_ids(self) -> None:
        ...
        for ap_cam in self.camera_objects:
            if ap_cam.name not in cam_ids:
                ...
                raise ValueError(f'You added multiple cameras with the cam_id {ap_cam.name}, '
    ...
    def _save_calibration(self) -> None:
        calibration_output_filepath = self.output_directory.joinpath(f'calibration_{self.recording_date.strftime("%y%m%d")}.toml')
...
class Triangulation(ABC):
    ...
    def _load_calibration(self, filepath: Path) -> None:
        if filepath.name.endswith('.toml') and filepath.exists():
            ...
            raise FileNotFoundError (f'The path, given as calibration_toml_filepath\n'
    ...
    def _preprocess_dlc_predictions_for_anipose(self) -> None:
        anipose_io = ap_lib.utils.load_pose2d_fnames(fname_dict = self.triangulation_dlc_cams_filepaths)
        self.anipose_io = self._add_additional_information_and_continue_preprocessing(anipose_io = anipose_io)
...
class Triangulation_Recordings(Triangulation):
    ...
    def __init__(self, recording_directory: Path, calibration_toml_filepath: Path, config_filepath: Path, max_frame_count: int, load_calibration: bool = False, output_directory: Optional[Path] = None) -> None:
        self.recording_directory = recording_directory
        self._load_calibration(filepath = calibration_toml_filepath)
        self._create_video_objects(recording_directory = recording_directory, config_filepath = config_filepath, load_calibration = load_calibration, max_frame_count = max_frame_count)
        self._validate_unique_cam_ids()
    ...
    def _create_video_objects(self, recording_directory: Path, config_filepath: Path, max_frame_count: int, load_calibration: bool = False) -> None:
        ...
        recording_metadata = [VideoMetadata(filepath = filepath, config_filepath = config_filepath, load_calibration = load_calibration, max_frame_count = max_frame_count) for filepath in recording_videofiles]
    ...
    def _validate_and_save_metadata_for_recording(self) -> None:
        ...
        if all([metadata.recording_date for metadata in self.metadata_from_videos]) and all([metadata.paradigm for metadata in self.metadata_from_videos]) and all([metadata.mouse_id for metadata in self.metadata_from_videos]):
            self.recording_date = self.metadata_from_videos[0].recording_date
            self.paradigm = self.metadata_from_videos[0].paradigm
            self.mouse_id = self.metadata_from_videos[0].mouse_id
            self.csv_output_filepath = self.output_directory.joinpath(f'{self.mouse_id}_{self.recording_date.strftime("%y%m%d")}_{self.paradigm}.csv')
        else:
            raise ValueError (f'The metadata, that was read from the videos in {self.recording_directory}, is not identical.\n'
                             'Please check the files and rename them properly!')
...
class Triangulation_Positions(Triangulation):
    ...
    def _validate_unique_cam_ids():
        ...
        # possibility to create empty .h5 for missing recordings?
        if self.triangulation_dlc_cams_filepaths.keys().sort() != self.cameras.sort():
            raise ValueError (f'The cam_ids of the recordings in {self.positions_directory} do not match the cam_ids of the camera_group at {self.calibration_toml_filepath}.\n'
                              'Are there missing or additional files in the calibration or the recording folder?')
    
    def _validate_and_save_metadata_for_recording():
        if all([metadata.recording_date for metadata in self.metadata_from_videos]):
            self.recording_date = self.metadata_from_videos[0].recording_date
            self.csv_output_filepath = self.positions_directory.joinpath(f'Positions_{self.recording_date.strftime("%y%m%d")}.csv')
        else:
    ...
    def _add_reprojection_errors_of_all_test_position_markers(self) -> None:
        ...
        for key in self.df.iloc[0].keys():
            if "error" in key:
                ...
                self.reprojection_errors_test_position_markers[marker_id] = reprojection_error # since we only have a single image
        ...
        self.reprojection_errors_test_position_markers['mean'] = np.asarray(all_reprojection_errors).mean() 
    
    def _add_all_real_distances_errors(self, test_positions_gt: TestPositionsGroundTruth) -> None:
        all_distance_to_cm_conversion_factors = self._get_conversion_factors_from_different_references(test_positions_gt = test_positions_gt)
        self._add_distances_in_cm_for_each_conversion_factor(anipose_io = anipose_io, conversion_factors = all_distance_to_cm_conversion_factors)
        self._add_distance_errors(gt_distances = test_positions_gt.marker_ids_with_distances)
        
    def _get_conversion_factors_from_different_references(self, test_positions_gt: TestPositionsGroundTruth) -> Dict: # Tuple? List?
    ...
    def _get_xyz_to_cm_conversion_factor(self, reference_marker_ids: Tuple[str, str], distance_in_cm: Union[int, float]) -> float:
    ...
    def _get_xyz_distance_in_triangulation_space(self, marker_ids: Tuple[str, str]) -> float:
        squared_differences = [(self.df[f'{marker_ids[0]}_{axis}'] - self.df[f'{marker_ids[1]}_{axis}'])**2 for axis in ['x', 'y', 'z']]
    ...
    def _add_distances_in_cm_for_each_conversion_factor(self, conversion_factors: Dict):
        ...
        for reference_distance_id, conversion_factor in conversion_factors.items():
            self.distances_in_cm[reference_distance_id] = self._convert_all_xyz_distances(conversion_factor = conversion_factor)
    ...
    def _add_distance_errors(self, gt_distances: Dict) -> None:
        ...
        for reference_distance_id, triangulated_distances in self.distances_in_cm.items():
            ...
            self.distance_errors_in_cm[reference_distance_id] = {'individual_errors': marker_ids_with_distance_error,
