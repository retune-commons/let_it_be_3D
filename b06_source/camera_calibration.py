...
class IntrinsicCameraCalibrator(ABC):
    ...
    def objp(self) -> np.ndarray:
        objp = np.zeros((1, self.checkerboard_rows_and_columns[0] * self.checkerboard_rows_and_columns[1], 3),
                        np.float32)
        objp[0, :, :2] = np.mgrid[0:self.checkerboard_rows_and_columns[0],
                         0:self.checkerboard_rows_and_columns[1]].T.reshape(-1, 2)
        return objp
    ...
    def run(self) -> Dict:
        ...
        retval, K, D, rvec, tvec = self._run_camera_type_specific_calibration(objpoints=object_points,
                                                                              imgpoints=detected_checkerboard_corners_per_image)
    ...
    def _attempt_to_match_max_frame_count(self, corners_per_image: List[np.ndarray],
                                          already_selected_frame_idxs: List[int]) -> List[np.ndarray]:
    ...
    def _attempt_to_reach_max_frame_count(self, corners_per_image: List[np.ndarray],
                                          already_selected_frame_idxs: List[int]) -> List[np.ndarray]:
        # ToDo
        # limit time?
...
class IntrinsicCalibratorFisheyeCamera(IntrinsicCameraCalibrator):
    ...
    def _run_camera_type_specific_calibration(self, objpoints: List[np.ndarray], imgpoints: List[np.ndarray]) -> Tuple:
        ...
        return cv2.fisheye.calibrate(objpoints, imgpoints, self.imsize, self.k, self.d, rvecs, tvecs, calibration_flags,
...
class TestPositionsGroundTruth:
    ...
    def add_new_marker_id(self, marker_id: str,
                          other_marker_ids_with_distances: List[Tuple[str, Union[int, float]]]) -> None:
    ...
    def add_marker_ids_and_distance_id_as_reference_distance(self, marker_ids: Tuple[str, str],
...
class SingleCamDataForAnipose:
    ...
    def add_manual_test_position_marker(self, marker_id: str, x_or_column_idx: int, y_or_row_idx: int,
                                        likelihood: float, overwrite: bool = False) -> None:
        ...
        self.manual_test_position_marker_coords_pred[marker_id] = {'x': [x_or_column_idx], 'y': [y_or_row_idx],
    ...
    def inspect_intrinsic_calibration(self, frame_idx: int = 0) -> None:
        distorted_input_image = load_single_frame_of_video(filepath=self.filepath_synchronized_calibration_video,
                                                           frame_idx=frame_idx)
        if self.fisheye:
        ...
        self._plot_distorted_and_undistorted_image(distorted_image=distorted_input_image,
    ...
    def load_intrinsic_camera_calibration(self, filepath_intrinsic_calibration: Path) -> None:
        ...
        self._set_intrinsic_calibration(intrinsic_calibration=intrinsic_calibration,
    ...
    def run_intrinsic_camera_calibration(self, filepath_checkerboard_video: Path, save: bool = True,
                                         max_frame_count: int = 300) -> None:
        if self.fisheye:
            calibrator = IntrinsicCalibratorFisheyeCamera(filepath_calibration_video=filepath_checkerboard_video,
                                                          max_frame_count=max_frame_count)
        else:
            calibrator = IntrinsicCalibratorRegularCamera(filepath_calibration_video=filepath_checkerboard_video,
                                                          max_frame_count=max_frame_count)
    ...
    def validate_and_save_manual_marker_coords_as_fake_dlc_output(self, test_positions_gt: TestPositionsGroundTruth,
        ...
        self.validate_test_position_marker_ids(test_positions_gt=test_positions_gt,
    ...
    def validate_test_position_marker_ids(self, test_positions_gt: TestPositionsGroundTruth,
                                          add_missing_marker_ids_with_0_likelihood: bool = True) -> None:
        ...
        prediction_marker_ids = list(
            set([marker_id for scorer, marker_id, key in self.test_position_markers_df.columns]))
    ...
    def _adjust_intrinsic_calibration(self, unadjusted_intrinsic_calibration: Dict) -> Dict:
        ...
        x_offset, y_offset = self._get_correct_x_y_offsets(
            intrinsic_calibration_video_size=intrinsic_calibration_video_size, new_video_size=new_video_size)
    ...
    def _get_correct_x_y_offsets(self, intrinsic_calibration_video_size: Tuple[int, int],
                                 new_video_size: Tuple[int, int]) -> Tuple[int, int]:
        # ToDo:
    ...
    def _incorporate_adjustments_in_intrinsic_calibration(self, intrinsic_calibration: Dict, new_size: Tuple[int, int],
                                                          adjusted_K: np.ndarray) -> Dict:
...
class CalibrationForAnipose3DTracking:
    ...
    def run_calibration(self, use_own_intrinsic_calibration: bool = True,
                        charuco_calibration_board: Optional[ap_lib.boards.CharucoBoard] = None) -> None:
        # ToDo
        ...
        if charuco_calibration_board == None:
            ...
            charuco_calibration_board = ap_lib.boards.CharucoBoard(7, 5, square_length=1, marker_length=0.8,
    ...
    def _initialize_camera_group(self) -> None:
        all_Camera_objects = [single_cam.export_as_aniposelib_Camera_object(single_cam.fisheye) for single_cam in
                              self.single_cam_objects]
    ...
    def _postprocess_triangulations_and_calculate_reprojection_error(self, anipose_io: Dict) -> Dict:
        anipose_io['reprojerr_flat'] = self.camera_group.reprojection_error(anipose_io['p3ds_flat'],
                                                                            anipose_io['points_flat'], mean=True)
    ...
    def _validate_test_position_markers_df_is_loaded_to_all_single_cam_objects(self, single_cams_to_calibrate: List[
        SingleCamDataForAnipose]):
    ...
    def run_calibration_control(self, show_full_output=False) -> None:
        ...
        angle_at_open_left = self._get_angle_between_three_points_at_PointA(PointA=maze_corner_open_right,
                                                                            PointB=maze_corner_closed_left,
                                                                            PointC=maze_corner_open_left)
        angle_at_closed_right = self._get_angle_between_three_points_at_PointA(PointA=maze_corner_closed_left,
