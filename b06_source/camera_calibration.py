...
class IntrinsicCameraCalibrator(ABC):
    ...
    def run(self) -> Dict:
        ...
        detected_checkerboard_corners_per_image = self._detect_checkerboard_corners(
...
class SingleCamDataForAnipose:
    ...
    def __init__(
        self,
        cam_id: str,
        filepath_synchronized_calibration_video: Path,
        fisheye: bool = False,
    ) -> None:
        self.cam_id = cam_id
        self.fisheye = fisheye
        self.filepath_synchronized_calibration_video = (
            filepath_synchronized_calibration_video
        )

    def add_cropping_offsets(
        self, x_or_column_offset: int = 0, y_or_row_offset: int = 0
    ) -> None:
        setattr(self, "cropping_offsets", (x_or_column_offset, y_or_row_offset))

    def add_flipping_details(
        self, flipped_horizontally: bool = False, flipped_vertically: bool = False
    ) -> None:
    ...
    def add_manual_test_position_marker(
        self,
        marker_id: str,
        x_or_column_idx: int,
        y_or_row_idx: int,
        ...
        if (marker_id in self.manual_test_position_marker_coords_pred.keys()) & (
            overwrite == False
        ):
            raise ValueError(
                "There are already coordinates for the marker you "
        ...
        self.manual_test_position_marker_coords_pred[marker_id] = {
            "x": [x_or_column_idx],
            "y": [y_or_row_idx],
            "likelihood": [likelihood],
        }

    def export_as_aniposelib_Camera_object(
        self, fisheye: bool
    ) -> ap_lib.cameras.Camera:
        if fisheye:
    ...
    def inspect_intrinsic_calibration(self, frame_idx: int = 0) -> None:
        distorted_input_image = load_single_frame_of_video(
            filepath=self.filepath_synchronized_calibration_video, frame_idx=frame_idx
        )
        if self.fisheye:
    ...
    def load_intrinsic_camera_calibration(
        self, filepath_intrinsic_calibration: Path
    ) -> None:
    ...
    def load_test_position_markers_df_from_dlc_prediction(
        self, filepath_deeplabcut_prediction: Path
    ) -> None:
        df = pd.read_hdf(filepath_deeplabcut_prediction)
        setattr(self, "test_position_markers_df", df)
        setattr(
            self,
            "filepath_test_position_marker_prediction",
            filepath_deeplabcut_prediction,
        )

    def run_intrinsic_camera_calibration(
        self,
        filepath_checkerboard_video: Path,
        save: bool = True,
        max_frame_count: int = 300,
    ) -> None:
        if self.fisheye:
            calibrator = IntrinsicCalibratorFisheyeCamera(
                filepath_calibration_video=filepath_checkerboard_video,
                max_frame_count=max_frame_count,
            )
        else:
            calibrator = IntrinsicCalibratorRegularCamera(
                filepath_calibration_video=filepath_checkerboard_video,
                max_frame_count=max_frame_count,
            )
    ...
    def validate_and_save_manual_marker_coords_as_fake_dlc_output(
        self,
        test_positions_gt: TestPositionsGroundTruth,
        ...
        if output_filepath.name.endswith(".h5") == False:
            if output_filepath.is_dir():
                output_filepath = output_filepath.joinpath(
                    f"{self.cam_id}_manual_test_position_marker_fake.h5"
                )
            else:
                output_filepath = output_filepath.parent.joinpath(
                    f"{self.cam_id}_manual_test_position_marker_fake.h5"
                )
        self.test_position_markers_df = (
            self._construct_dlc_output_style_df_from_manual_marker_coords()
        )
        self.validate_test_position_marker_ids(
            test_positions_gt=test_positions_gt,
            add_missing_marker_ids_with_0_likelihood=add_missing_marker_ids_with_0_likelihood,
        )
        self.test_position_markers_df.to_hdf(output_filepath, "df")
        print(
            f"Your dataframe was successfully saved at: {output_filepath.as_posix()}."
        )
        self.load_test_position_markers_df_from_dlc_prediction(
            filepath_deeplabcut_prediction=output_filepath
        )
        print(
            'Your "fake DLC marker perdictions" were successfully loaded to the SingleCamDataForAnipose object!'
        )

    def save_manual_marker_coords_as_fake_dlc_output(
    ...
    def validate_test_position_marker_ids(
        self,
        test_positions_gt: TestPositionsGroundTruth,
        add_missing_marker_ids_with_0_likelihood: bool = True,
    ) -> None:
        if hasattr(self, "test_position_markers_df") == False:
            raise ValueError(
                "There was no DLC prediction of the test position markers loaded yet. "
        ...
        prediction_marker_ids = list(
            set(
                [
                    marker_id
                    for scorer, marker_id, key in self.test_position_markers_df.columns
                ]
            )
        )
        marker_ids_not_in_ground_truth = self._find_non_matching_marker_ids(
            prediction_marker_ids, ground_truth_marker_ids
        )
        marker_ids_not_in_prediction = self._find_non_matching_marker_ids(
            ground_truth_marker_ids, prediction_marker_ids
        )
    ...
    def _add_missing_marker_ids_to_prediction(
        self, missing_marker_ids: List[str]
    ) -> None:
    ...
    def _adjust_intrinsic_calibration(
        ...
        x_offset, y_offset = self._get_correct_x_y_offsets(
            intrinsic_calibration_video_size=intrinsic_calibration_video_size,
            new_video_size=new_video_size,
        )
        adjusted_K = self._get_adjusted_K(
            K=unadjusted_intrinsic_calibration["K"],
            x_offset=x_offset,
            y_offset=y_offset,
        )
    ...
    def _check_if_all_details_were_provided_otherwise_set_defaults(self) -> None:
        attributes_and_default_value_setters = {
            "cropping_offsets": self.add_cropping_offsets,
        ...
        for (
            attribute_name,
            default_setter,
        ) in attributes_and_default_value_setters.items():
            if hasattr(self, attribute_name) == False:
                ...
                print(
                    f'User info: since no other information were provided, "{key_to_print}" '
    ...
    def _construct_dlc_output_style_df_from_manual_marker_coords(self) -> pd.DataFrame:
        ...
        for scorer, marker_id, key in df.columns:
            df[(scorer, marker_id, key)] = self.manual_test_position_marker_coords_pred[
                marker_id
            ][key]
        return df
    ...
    def _get_adjusted_K(
        self, K: np.ndarray, x_offset: int, y_offset: int
    ) -> np.ndarray:
        adjusted_K = K.copy()
    ...
    def _get_multi_index(self) -> pd.MultiIndex:
        ...
        for marker_id in self.manual_test_position_marker_coords_pred.keys():
            for column_name in ("x", "y", "likelihood"):
                multi_index_column_names[0].append(
                    "manually_annotated_marker_positions"
                )
    ...
    def _is_adjusting_of_intrinsic_calibration_required(
        ...
        if any(
            [
                self.cropping_offsets != (0, 0),
                self.flipped_horizontally,
                self.flipped_vertically,
                self.degrees_rotated_clockwise != 0,
            ]
        ):
    ...
    def _remove_marker_ids_not_in_ground_truth(
        self, marker_ids_to_remove: List[str]
    ) -> None:
        df = self.test_position_markers_df
        columns_to_remove = [
            column_name
...
class CalibrationForAnipose3DTracking:
    def __init__(self, single_cams_to_calibrate: List[SingleCamDataForAnipose]) -> None:
        ...
        self._validate_test_position_markers_df_is_loaded_to_all_single_cam_objects(
            single_cams_to_calibrate=single_cams_to_calibrate
        )
    ...
    def evaluate_triangulation_of_test_position_markers(
        self, test_positions_gt: Dict, show_3D_plot: bool = True, verbose: bool = True
    ) -> None:
        self.anipose_io = self._preprocess_dlc_predictions_for_anipose()
        self.anipose_io["p3ds_flat"] = self.camera_group.triangulate(
            self.anipose_io["points_flat"], progress=True
        )
        self.anipose_io = self._postprocess_triangulations_and_calculate_reprojection_error(
            anipose_io=self.anipose_io
        )
        self.anipose_io = self._add_dataframe_of_triangulated_points(
            anipose_io=self.anipose_io
        )
        self.anipose_io = self._add_reprojection_errors_of_all_test_position_markers(
            anipose_io=self.anipose_io
        )
        ...
        self.anipose_io = self._add_all_real_distances_errors(
            anipose_io=self.anipose_io, test_positions_gt=gt_distances
        )
        ...
        if show_3D_plot:
            self._show_3D_plot(
                frame_idx=0,
                anipose_io=self.anipose_io,
                marker_ids_to_connect=test_positions_gt[
    ...
    def run_calibration(
        ...
        self.camera_group.calibrate_videos(
            videos=self.calibration_video_filepaths,
    ...
    def _set_distances_from_configuration(self, distances_to_compute):
        conversion_factors = self._get_conversion_factors_from_different_references(
            self.anipose_io, distances_to_compute
        )
        self._add_distances_in_cm_for_each_conversion_factor(
            self.anipose_io, conversion_factors
        )
        return
    ...
    def _add_all_real_distances_errors(
        self, anipose_io: Dict, test_positions_gt: Dict
    ) -> Dict:
        ...
        anipose_io = self._add_distances_in_cm_for_each_conversion_factor(
            anipose_io=anipose_io,
            conversion_factors=all_distance_to_cm_conversion_factors,
        )
        anipose_io = self._add_distance_errors(
            anipose_io=anipose_io, gt_distances=test_positions_gt
        )
        return anipose_io
    ...
    def _computes_angles(self, angles_to_compute) -> Dict[str, float]:
        ...
        for angle, markers_dictionary in angles_to_compute.items():
            if len(markers_dictionary["marker"]) == 3:
                pt_a = self._get_vector_from_label(
                    label=markers_dictionary["marker"][0]
                )
                pt_b = self._get_vector_from_label(
                    label=markers_dictionary["marker"][1]
                )
                pt_c = self._get_vector_from_label(
                    label=markers_dictionary["marker"][2]
                )
                triangulated_angles[
                    angle
                ] = self._get_angle_between_three_points_at_PointA(
                ...
                pt_b = self._get_vector_from_label(
                    label=markers_dictionary["marker"][3]
                )
                pt_c = self._get_vector_from_label(
                    label=markers_dictionary["marker"][4]
                )
                plane_coord = self._get_coordinates_plane_equation_from_three_points(
                    PointA=pt_a, PointB=pt_b, PointC=pt_c
                )
                N = self._get_vector_product(A=plane_coord[0], B=plane_coord[2])

                pt_d = self._get_vector_from_label(
                    label=markers_dictionary["marker"][0]
                )
                pt_e = self._get_vector_from_label(
                    label=markers_dictionary["marker"][1]
                )
                triangulated_angles[
                    angle
                ] = self._get_angle_between_two_points_and_plane(
    ...
    def _set_angles_error_between_screws_and_plane(self, gt_angles):
        ...
        self.anipose_io[
            "angles_error_screws_plan"
        ] = self._compute_differences_between_triangulated_and_gt_angles(gt_angles)
    ...
    def _convert_all_xyz_distances(
        self, anipose_io: Dict, conversion_factor: float
    ) -> Dict:
    ...
    def _get_all_calibration_video_filepaths(self) -> None:
        video_filepaths = [
            [single_cam.filepath_synchronized_calibration_video.as_posix()]
    ...
    def _get_conversion_factors_from_different_references(
        ...
        for reference, markers in test_positions_gt.items():
            for m in markers:
    ...
    def _get_xyz_distance_in_triangulation_space(
        self, marker_ids: Tuple[str, str], df_xyz: pd.DataFrame
    ) -> float:
        squared_differences = [
    ...
    def _initialize_camera_group(self) -> None:
        all_Camera_objects = [
            single_cam.export_as_aniposelib_Camera_object(single_cam.fisheye)
            for single_cam in self.single_cam_objects
        ]
    ...
    def _preprocess_dlc_predictions_for_anipose(self) -> Dict:
        ...
        for single_cam in self.single_cam_objects:
            fname_dict[
                single_cam.cam_id
            ] = single_cam.filepath_test_position_marker_prediction
        anipose_io = ap_lib.utils.load_pose2d_fnames(fname_dict=fname_dict)
        anipose_io = self._add_additional_information_and_continue_preprocessing(
            anipose_io=anipose_io
        )
        return anipose_io
    ...
    def _postprocess_triangulations_and_calculate_reprojection_error(
        ...
        anipose_io["reprojerr_flat"] = self.camera_group.reprojection_error(
            anipose_io["p3ds_flat"], anipose_io["points_flat"], mean=True
        )
        anipose_io["p3ds"] = anipose_io["p3ds_flat"].reshape(
            anipose_io["n_points"], anipose_io["n_joints"], 3
        )
        anipose_io["reprojerr"] = anipose_io["reprojerr_flat"].reshape(
            anipose_io["n_points"], anipose_io["n_joints"]
        )
        anipose_io["reproj_nonan"] = anipose_io["reprojerr"][
            np.logical_not(np.isnan(anipose_io["reprojerr"]))
        ]
        return anipose_io

    def _show_3D_plot(
        self, frame_idx: int, anipose_io: Dict, marker_ids_to_connect: List[Tuple[str]]
    ) -> None:
        ...
        self._connect_all_marker_ids(
            ...
            bodyparts=anipose_io["bodyparts"],
        )
        for i in range(len(anipose_io["bodyparts"])):
            ax.text(
                ...
                p3d[i, 1] + 0.01,
                p3d[i, 2],
                anipose_io["bodyparts"][i],
                size=9,
            )
        plt.show()

    def _validate_test_position_markers_df_is_loaded_to_all_single_cam_objects(
        self, single_cams_to_calibrate: List[SingleCamDataForAnipose]
    ):
        ...
        for single_cam in single_cams_to_calibrate:
            if hasattr(single_cam, "test_position_markers_df") == False:
                raise ValueError(
                    "For this evaluation, all SingleCamDataForAnipose objects must have "
    ...
    def _validate_unique_cam_ids(
        ...
        for single_cam in single_cams_to_calibrate:
            if single_cam.cam_id not in cam_ids:
                ...
                raise ValueError(
                    f"You added multiple cameras with the cam_id {single_cam.cam_id}, "
    ...
    def _get_angle_from_law_of_cosines(
        self, length_a: float, length_b: float, length_c: float
    ) -> float:
    ...
    def _get_angle_between_three_points_at_PointA(
        self, PointA: np.array, PointB: np.array, PointC: np.array
    ) -> float:
    ...
    def _get_Richtungsvektor_from_two_points(
        self, PointA: np.array, PointB: np.array
    ) -> np.array:
        R = np.asarray(
    ...
    def _get_angle_between_two_points_and_plane(
        self, PointA: np.array, PointB: np.array, N: np.array
    ) -> float:
    ...
    def _get_distance_between_plane_and_point(
        self, N: np.array, PointOnPlane: np.array, DistantPoint: np.array
    ) -> float:
    ...
    def _get_vector_from_label(self, label: str) -> np.array:
        return np.asarray(
            [
                self.anipose_io["df_xyz"][label + "_x"],
                self.anipose_io["df_xyz"][label + "_y"],
    ...
    def run_calibration_control(self, show_full_output=False) -> None:
        ...
        maze_corner_closed_left = self._get_vector_from_label(
            label="MazeCornerClosedLeft"
        )
        maze_corner_open_left = self._get_vector_from_label(label="MazeCornerOpenLeft")
        maze_corner_closed_right = self._get_vector_from_label(
            label="MazeCornerClosedRight"
        )
        maze_corner_open_right = self._get_vector_from_label(
            label="MazeCornerOpenRight"
        )

        angle_at_open_right = self._get_angle_between_three_points_at_PointA(
            PointA=maze_corner_open_left,
            PointB=maze_corner_closed_right,
            PointC=maze_corner_open_right,
        )
        angle_at_open_left = self._get_angle_between_three_points_at_PointA(
            PointA=maze_corner_open_right,
            PointB=maze_corner_closed_left,
            PointC=maze_corner_open_left,
        )
        angle_at_closed_right = self._get_angle_between_three_points_at_PointA(
            PointA=maze_corner_closed_left,
            PointB=maze_corner_open_right,
            PointC=maze_corner_closed_right,
        )
        angle_at_closed_left = self._get_angle_between_three_points_at_PointA(
            PointA=maze_corner_open_left,
            PointB=maze_corner_closed_right,
        ...
        angle_screw_1 = self._get_angle_between_two_points_and_plane(
            PointA=screw1_bottom, PointB=screw1_top, N=N
        )
        screw2_bottom = self._get_vector_from_label(label="Screw2Bottom")
        screw2_top = self._get_vector_from_label(label="Screw2Top")
        angle_screw_2 = self._get_angle_between_two_points_and_plane(
            PointA=screw2_bottom, PointB=screw2_top, N=N
        )
        screw3_bottom = self._get_vector_from_label(label="Screw3Bottom")
        screw3_top = self._get_vector_from_label(label="Screw3Top")
        angle_screw_3 = self._get_angle_between_two_points_and_plane(
            PointA=screw3_bottom, PointB=screw3_top, N=N
        )
        screw4_bottom = self._get_vector_from_label(label="Screw4Bottom")
        screw4_top = self._get_vector_from_label(label="Screw4Top")
        angle_screw_4 = self._get_angle_between_two_points_and_plane(
            PointA=screw4_bottom, PointB=screw4_top, N=N
        )

        print(
            f"Angle of objects on the maze:\n"
        ...
        distance_X1_to_maze_plane = self._get_distance_between_plane_and_point(
            N=N, PointOnPlane=plane_coord[0], DistantPoint=x1
        )[0]
        distance_X2_to_maze_plane = self._get_distance_between_plane_and_point(
            N=N, PointOnPlane=plane_coord[0], DistantPoint=x2
        )[0]
        # in cm?

        print(
            f"Distance of X1 and X2:\n"
