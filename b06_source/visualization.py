...
class Visualization():
    ...
    def __init__(self, frame_idx: int, filepath_to_3D_csv: Path, marker_ids_to_connect: List[Tuple[str]], paradigm: Optional[str] = None, return_frame: bool = False, plot_in_cm: bool = False):
        self._read_csv(filepath_to_3D_csv = filepath_to_3D_csv)
        self.plot_in_cm = plot_in_cm
        self._show_3D_plot(frame_idx = frame_idx, marker_ids_to_connect = marker_ids_to_connect, paradigm = paradigm, return_frame = return_frame)
    ...
    def _read_csv(self, filepath_to_3D_csv: Path):
        ...
        for key in self.df.keys():
            ...
            if bodypart not in self.bodyparts and bodypart not in set (['M', 'center', 'fnum']):
                self.bodyparts.append(bodypart)
    
    def _show_3D_plot(self, frame_idx: int, marker_ids_to_connect: List[Tuple[str]], paradigm: Optional[str] = None, return_frame: bool = False) -> None:
        ...
        for bodypart in self.bodyparts:
            if bodypart not in set(['LED5', 'MazeCornerOpenRight', 'MazeCornerOpenLeft', 'MazeCornerClosedRight', 'MazeCornerClosedLeft']):
                if not math.isnan(self.df.loc[frame_idx, f'{bodypart}_x']):
                    ax.text(self.df.loc[frame_idx, f'{bodypart}_x'], self.df.loc[frame_idx, f'{bodypart}_y'], self.df.loc[frame_idx, f'{bodypart}_z'], bodypart, size = 7)
                    ax.scatter(self.df.loc[frame_idx, f'{bodypart}_x'], self.df.loc[frame_idx, f'{bodypart}_y'], self.df.loc[frame_idx, f'{bodypart}_z'], s=10, alpha = 1)
            
        self._connect_all_marker_ids(ax = ax, scheme = marker_ids_to_connect, frame_idx = frame_idx)
        if self.plot_in_cm:
    ...
    def _connect_all_marker_ids(self, ax: plt.Figure, scheme: List[Tuple[str]], frame_idx: int) -> List[plt.Figure]:
        # ToDo: correct type hints
        cmap = plt.get_cmap('tab10')
        bp_dict = dict(zip(self.bodyparts, range(len(self.bodyparts))))
        for i, bps in enumerate(scheme):
            self._connect_one_set_of_marker_ids(ax = ax, bps = bps, bp_dict = bp_dict, color = cmap(i)[:3], frame_idx = frame_idx)
    
    def _connect_one_set_of_marker_ids(self, ax: plt.Figure, bps: List[str], bp_dict: Dict, color: np.ndarray, frame_idx: int) -> plt.Figure:
        # ToDo: correct type hints
        x = [f'{bodypart}_x' for bodypart in bps]
        y = [f'{bodypart}_y' for bodypart in bps]
        z = [f'{bodypart}_z' for bodypart in bps]
        ax.plot(self.df.loc[frame_idx, x], self.df.loc[frame_idx, y], self.df.loc[frame_idx, z], color = color)
